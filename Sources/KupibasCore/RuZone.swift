...
public enum RuZone {
    ...
    public static func networks(extraPaths: [String] = []) -> [Ipv4Net] {
        ...
        // Пустоту не запоминаем. Служба может обратиться к списку раньше,
        // чем установщик успел его положить, — и тогда запомненная пустота
        // означала бы, что обход российской зоны не заработает до
        // перезапуска службы: банки и госуслуги ушли бы через VPN.
        return []
    }
